...
import { QuotaAmount } from '@/components/quota-amount'
...
function getSummaryValue(
  ...
  if (key === 'todayUsage') return <QuotaAmount value={recentUsage} />
  if (key === 'usage') return <QuotaAmount value={usedQuota} />
  return value
}
...
export function SummaryCards() {
  ...
  const customCurrency = currency.quotaDisplayType === 'CUSTOM'
  ...
  const items = useSummaryCardsConfig({
    ...summaryValues,
    todayUsageDisplay,
    currencyEnabled: currencyEnabled && !customCurrency,
    currencyLabel,
  }).map((config, index) => {
    ...
    return {
      key: config.key,
      title: config.title,
      value: getSummaryValue(config.key, config.value, recentUsage, usedQuota),
  ...
  return (
    <div className='scaling-overview-summary bg-card overflow-hidden rounded-xl border shadow-xs'>
      <div className='scaling-overview-summary-grid grid xl:grid-cols-[minmax(0,1fr)_19rem]'>
        <div className='flex flex-col gap-2.5 p-3 sm:gap-3 sm:p-5'>
          ...
          <StaggerContainer className='scaling-overview-metrics grid grid-cols-3 gap-1.5 sm:gap-3'>
            {items.map((it) => (
              <StaggerItem
                key={it.key}
                className='scaling-overview-stat px-2 py-1.5 sm:p-3'
              >
                <StatCard
                  title={it.title}
                  value={it.value}
        ...
        <div className='scaling-overview-balance flex flex-col justify-between gap-3 border-t bg-[linear-gradient(135deg,color-mix(in_oklch,var(--overview-accent-2)_12%,var(--background))_0%,color-mix(in_oklch,oklch(0.82_0.04_155)_8%,var(--background))_48%,color-mix(in_oklch,var(--overview-accent-1)_7%,var(--background))_100%)] p-3 sm:gap-4 sm:p-5 xl:border-t-0 xl:border-l'>
          <div className='flex flex-col gap-2 sm:gap-3'>
            <div className='flex items-center justify-between'>
              ...
              >
                {customCurrency ? (
                  <span className='sr-only'>{t('Credit remaining')}</span>
                ) : (
                  <>
