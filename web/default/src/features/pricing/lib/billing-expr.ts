...
// Settlement traces are authoritative; never infer a historical multiplier
// from the current time or the current model configuration.
export function getAppliedRequestMultiplier(
  ...
  for (const trace of traces) {
    if (
      typeof trace.matched !== 'boolean' ||
      !Number.isFinite(trace.multiplier) ||
      trace.multiplier < 0
    ) {
      return null
    }
    if (trace.matched) multiplier *= trace.multiplier
  }
