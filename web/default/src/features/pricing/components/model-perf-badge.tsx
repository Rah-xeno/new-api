...
export const ModelPerfBadge = memo(function ModelPerfBadge(
  ...
  const hasSuccessRate =
    ...
    successRate >= 0 &&
    successRate <= 100
  // new-api returns an ordered sample array without timestamps.
  // Right-align available samples; leave missing slots empty.
  const statusRates = useMemo(() => {
    const rates = props.perf?.recent_success_rates?.slice(-24) ?? []
    return STATUS_SLOTS.map((slot) => rates[slot - (24 - rates.length)])
