  ...
  if (cond.source === SOURCE_TIME) {
    ...
    if (cond.mode === MATCH_RANGE) {
      if (cond.timeFunc === 'weekday') {
        const weekday = new Intl.DateTimeFormat(toIntlLocale(language), {
          weekday: 'long',
          timeZone: 'UTC',
        })
        const start = Number(cond.rangeStart)
        const end = (Number(cond.rangeEnd) + 6) % 7
        return `${weekday.format(new Date(Date.UTC(2023, 0, 1 + start)))}–${weekday.format(new Date(Date.UTC(2023, 0, 1 + end)))} (${tz})`
      }
      if (cond.timeFunc === 'hour') {
        return `${cond.rangeStart.padStart(2, '0')}:00–${cond.rangeEnd.padStart(2, '0')}:00 (${tz})`
      }
      return `${fn} ≥ ${cond.rangeStart} ${Number(cond.rangeStart) < Number(cond.rangeEnd) ? '&&' : '||'} ${fn} < ${cond.rangeEnd} (${tz})`
    }
...
export function DynamicPricingBreakdown({
  ...
  const appliedMultiplier = getAppliedRequestMultiplier(requestRules)
      ...
      {hasRules && (
        <div>
          <p className='text-muted-foreground mb-2 text-xs'>
            {t(
              'Displayed unit prices exclude conditional multipliers. Multiply by matching conditions and the group ratio when billing.'
            )}
          </p>
          {appliedMultiplier !== null && (
            <p className='mb-2 text-sm font-medium'>
              {t('Applied conditional multiplier')}: ×{appliedMultiplier}
            </p>
          )}
          <div
            className={
              ...
              return (
                ...
                >
                  ...
                  >
                    {group.multiplier}x
                    {group.matched != null &&
                      ` · ${t(isMatched ? 'Matched' : 'Not matched')}`}
                  </Badge>
                </li>
              )
            })}
