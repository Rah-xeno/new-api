...
export function ModelPriceCell(props: {
  ...
  let currencyLabel = getCurrencyLabel()
  if (currency.quotaDisplayType === 'TOKENS') currencyLabel = 'USD'
  if (customCurrency) currencyLabel = ''
  ...
  const dynamic = getDynamicPricingSummary(props.model, {
    ...options,
    tokenUnit,
    showCurrencySymbol: customCurrency,
        ...
        {metrics.map((metric) => (
          ...
          >
            ...
            <span
              ...
              title={metric.value}
            >
              <QuotaText>{metric.value}</QuotaText>
            </span>
          </span>
        ))}
      </span>
